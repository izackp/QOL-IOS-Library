...
public extension UITextView {
    ...
    func resizeToFitTextMethod5(simulatedFont:UIFont? = nil) {
        ...
        let newHeight = UITextView.heightForWidth(width - textContainerInset.left - textContainerInset.right, text: text ?? "", font: fontToUse!, edgeInsets: UIEdgeInsets.zero, numberOfLines: 100)
        let insets:CGFloat = self.textContainerInset.top + self.textContainerInset.bottom
        height = newHeight + insets
    }

    static func heightForWidth(_ width: CGFloat, text: String, font: UIFont, edgeInsets: UIEdgeInsets, numberOfLines: Int) -> CGFloat {
        let insettedSize = CGSize(width: max(width - edgeInsets.left - edgeInsets.right, 0), height: .greatestFiniteMagnitude)
        let options: NSStringDrawingOptions = [.usesLineFragmentOrigin, .truncatesLastVisibleLine]
        var height = text.boundingRect(with: insettedSize, options: options, attributes: [.font: font], context: nil).size.height
        if numberOfLines > 0 {
